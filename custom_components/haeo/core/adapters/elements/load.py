...
def _node_dual_values(
    ...
    The shadow price on an element power-balance constraint is emitted in
...
def _stats_outputs(
    ...
    Cost is integrated as ``cost[t] = power[t] * node_dual[t] * periods[t]``
    ($), where ``node_dual[t]`` is the source-node power-balance shadow price
    ($/kWh, energy-native LP formulation).
    ...
    cost_per_step = tuple(p * d * dt for p, d, dt in zip(power, node_dual, periods, strict=True))
