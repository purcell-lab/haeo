...
def test_daily_window_with_30min_periods_picks_48_steps() -> None:
    ...
    # 48 steps of (1kW * 0.5h) = 24 kWh; cost = 48 * (1 * 0.05 * 0.5) = 1.20
    # (energy-native LP: cost[t] = power * dual_$/kWh * dt).
    assert next(iter(load_outputs[LOAD_DAILY_ENERGY].values)) == pytest.approx(24.0)
    assert next(iter(load_outputs[LOAD_DAILY_COST].values)) == pytest.approx(1.20)
