...
def test_total_cost_matches_energy_times_node_dual_integral() -> None:
    ...
    assert tuple(outputs[LOAD_HORIZON_ENERGY].values)[-1] == pytest.approx(expected_energy, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_COST].values)[-1] == pytest.approx(expected_cost, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_RUNTIME].values)[-1] == pytest.approx(expected_runtime, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_AVERAGE_COST].values)[-1] == pytest.approx(
...
def test_daily_sensors_match_totals_when_horizon_is_under_24h() -> None:
    ...
    assert next(iter(outputs[LOAD_NEXT_24H_ENERGY].values)) == pytest.approx(
        tuple(outputs[LOAD_HORIZON_ENERGY].values)[-1], abs=1e-6
    )
    assert next(iter(outputs[LOAD_NEXT_24H_COST].values)) == pytest.approx(
        tuple(outputs[LOAD_HORIZON_COST].values)[-1], abs=1e-6
    )
    assert next(iter(outputs[LOAD_NEXT_24H_RUNTIME].values)) == pytest.approx(
        tuple(outputs[LOAD_HORIZON_RUNTIME].values)[-1], abs=1e-6
    )
    assert next(iter(outputs[LOAD_NEXT_24H_AVERAGE_COST].values)) == pytest.approx(
        tuple(outputs[LOAD_HORIZON_AVERAGE_COST].values)[-1], abs=1e-6
    )
...
def test_cost_integration_uses_energy_not_power_on_sub_hour_periods() -> None:
    ...
    assert tuple(outputs[LOAD_HORIZON_COST].values)[-1] == pytest.approx(expected_cost_energy_weighted, abs=1e-6)
    ...
    avg_cost = tuple(outputs[LOAD_HORIZON_AVERAGE_COST].values)[-1]
    assert 0.0 <= avg_cost <= max(_IMPORT_PRICES) + 0.05, (
...
def test_zero_dispatch_produces_zero_stats_and_zero_average_cost() -> None:
    ...
    assert tuple(outputs[LOAD_HORIZON_ENERGY].values)[-1] == pytest.approx(0.0, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_COST].values)[-1] == pytest.approx(0.0, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_RUNTIME].values)[-1] == pytest.approx(0.0, abs=1e-6)
    assert tuple(outputs[LOAD_HORIZON_AVERAGE_COST].values)[-1] == pytest.approx(0.0, abs=1e-6)
